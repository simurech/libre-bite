...
class LBite_Features {
	...
	private static $feature_definitions = array(
		...
		'enable_stampcard'          => array(
			'group'       => 'order_system',
			'default'     => false,
			'premium'     => true,
			'label'       => 'Stamp Card',
			'description' => 'One stamp per completed order above a minimum value; after a set number the guest gets a discount voucher. Guests grasp “every tenth order is cheaper” immediately — a points balance with a conversion rate they do not.',
		),
		'enable_promotions'         => array(
			'group'       => 'checkout',
			'default'     => false,
			'premium'     => true,
			'label'       => 'Promotions & Announcements',
			'description' => 'Rule-based offers that WooCommerce does not know by itself — buy several pay for fewer, a discount on a category, money off above a cart value — each limited to certain weekdays or times. Plus an announcement bar so the offer gets noticed.',
		),
		...
		'enable_swiss_vat'          => array(
			...
			'description' => 'Apply a different tax class per order type (Takeaway vs. Dine-in).',
		),
		'enable_order_type_selection' => array(
			...
			'description' => 'Show a Takeaway / Dine-in selector in the checkout form. When Multiple Tax Rates is active, also controls which tax rate applies.',
		),
	...
	public static function get_premium_only_options() {
		return array(
			...
			'lbite_promotions'               => array(),
			'lbite_promo_banner'             => array(),
			'lbite_stampcard_min_total'      => 0,
			'lbite_stampcard_target'         => 10,
			'lbite_stampcard_discount'       => 50,
			'lbite_stampcard_validity_days'  => 90,
			'lbite_stampcard_discount_type'      => 'percent',
			'lbite_stampcard_max_amount'         => 0,
			'lbite_stampcard_limit_categories'   => array(),
			'lbite_stampcard_limit_to_one_item'  => 0,
