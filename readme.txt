...
* New: Product-location assignment switched from an opt-in whitelist to an opt-out blacklist — products are now available at all locations by default, and only need to be disabled for specific locations. New locations automatically get all existing products. Existing assignments are migrated automatically.
* New: Product availability indicator on the shop grid and single product page, showing at a glance how many (and which) locations carry the product.
* New: Availability filter on shop and category pages lets customers pick a location to see what is available there — including before any location has been selected elsewhere, and without needing to commit to an order time.
* Fix: Holiday "All locations" checkbox selection was reset to nothing after saving.
* Fix: Availability filter and its styling/scripts now load correctly on product category pages (previously only loaded on the main shop page).
* Fix: POS variant selection now respects the WooCommerce default variation instead of always preselecting the first one.
* Fix: Missing German translations for the availability filter notice in some locale variants.
* Removed: Decorative title above the location selector tiles.
* Maintenance: Updated Freemius SDK to version 2.13.4.
* Maintenance: Changelog section trimmed to stay within WordPress.org's word limit; full history available on GitHub.
...
= 2.0.22 =
* Fix: Order creation time in the WooCommerce orders list now correctly shows the site's local time. Previously the time was shifted by the UTC offset due to a mismatch between WooCommerce's datetime attribute encoding and the timezone conversion logic.

= 2.0.21 =
* Update: Freemius SDK updated to version 2.13.2.

= 2.0.20 =
* Fix: Order creation time in the orders list now always displays in the WordPress site timezone, regardless of the browser's timezone setting.
